...
class eight_queens_Arpan(object):
    ...
    ##----------------------------------------------------------------##
    def chomosome_display(self, chromosome, maxFitness):
        print("Chromosome = {},  Fitness = {}"
              .format(str(chromosome), 
                      self.fitness_measure(chromosome, maxFitness)))
    ##----------------------------------------------------------------##

    ##----------------------------------------------------------------##
    def probability_measure(self, chromosome, maxFitness, fitness):
    ...
    def random_pick(self, population, probabilities):
        ...
        total = sum(w for c, w in population_probability)
    ...
    ##----------------------------------------------------------------##
    def genetic_queen(self, population, maxFitness, fitness):
        mutation_probability = 0.03
        new_population = []
        probabilities = [self.probability_measure(n, 
                         maxFitness, fitness) for n in population]
        for i in range(len(population)):
            ...
            self.chomosome_display(child, maxFitness)
...
def eight_queens_puzzle(nq):
    ...
    while not maxFitness in \
        [eight_queens_obj.fitness_measure(chrom, 
         maxFitness) for chrom in population]:
        print("=== Generation {} ===".format(gen_repro_cnt))
        population = eight_queens_obj.genetic_queen(
                         population, 
                         maxFitness,
                         eight_queens_obj.fitness_measure)
        print("")
        print("Maximum Fitness = {}"
            .format(max([eight_queens_obj.fitness_measure(n, 
                    maxFitness) for n in population])))
        gen_repro_cnt = gen_repro_cnt + 1

    chrom_out = []
    print("Solved in Generation {}!".format(gen_repro_cnt-1))
    for chrom in population:
        if (eight_queens_obj.fitness_measure(chrom, maxFitness) == maxFitness):
            print("");
            print("One of the solutions: ")
            chrom_out = chrom
            eight_queens_obj.chomosome_display(chrom, maxFitness)
