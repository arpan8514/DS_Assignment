...
KERNEL_SIZE = 5    # The number of words for convolutional filters to cover
NUM_FILTERS = 64  # The number of filters per filter/kernel size
POOL_SIZE = 4
...
# adding 1D-Convolution 
training_model.add(Conv1D(filters=NUM_FILTERS,
...
# adding Max Pooling
training_model.add(MaxPooling1D(pool_size=POOL_SIZE))


# adding LSTM 
training_model.add(LSTM(units = HIDDEN_LAYER_SIZE_1,
                   return_sequences = True,
                   activation = "tanh",
                   recurrent_activation = "hard_sigmoid",
                   input_shape = (TIMESTEPS, DATA_DIM))) # stateless LSTM

# below LSTM returns a sequence of vectors of dimension HIDDEN_LAYER_SIZE_2
training_model.add(LSTM(units=HIDDEN_LAYER_SIZE_2, return_sequences=True))

# below LSTM returns a single vector of dimension HIDDEN_LAYER_SIZE_3
training_model.add(LSTM(HIDDEN_LAYER_SIZE_3))
...
checkpoint = ModelCheckpoint(
        'CNN_Stacked_LSTM_weights_{epoch:03d}_{val_acc:.4f}.hdf5',
        monitor='val_acc',
        verbose=1,
        save_best_only=True,
        mode='auto')
...
score_file = "stacked_LSTM_score.txt"
